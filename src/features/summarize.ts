/**
 * Summarize feature — /summarize command to generate an LLM conversation
 * summary displayed in a custom Markdown UI.
 */
...
const showSummaryUi = async (summary: string, ctx: ExtensionCommandContext) => {
    ...
    await ctx.ui.custom((_tui, theme, _kb, done) => {
        ...
        container.addChild(
            new Text(
                theme.fg("accent", theme.bold("Conversation Summary")),
                1,
                0
            )
        );
